// credits: https://github.com/brookhong/Surfingkeys/blob/master/src/content_scripts/common/utils.js
...
function listElements(root, whatToShow, filter) {
  ...
  while ((currentNode = nodeIterator.nextNode())) {
    if (currentNode.textContent == "Workspace") {
      console.log({ filterResult: filter(currentNode) });
    }
...
function LOG(level, msg) {
  ...
  chrome.storage.local.get(["logLevels"], (r) => {
    ...
    if (
      ["log", "warn", "error"].indexOf(level) !== -1 &&
      logLevels.indexOf(level) !== -1
    ) {
      console[level](msg);
    }
  });
...
function getWordUnderCursor(mouseCursor) {
  ...
  if (selection.focusNode && selection.focusNode.textContent) {
    ...
    if (selRect && word) {
      if (
        !mouseCursor ||
        (_clickPos && selRect.has(_clickPos[0], _clickPos[1], 0, 0))
      ) {
        return word.trim();
      }
    }
